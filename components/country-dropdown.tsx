...
export const CountryDropdown = ({}: CountryDropdownProps) => {
    ...
    const countryData: Country[] = React.useMemo(
        () =>
            countries
                .slice()
                .sort((a, b) =>
    ...
    const [selectedCountry, setSelectedCountry] = React.useState<Country>(getDefaultCountry(settings.countryCode));
    ...
    return (
        <Select defaultValue={{value: selectedCountry.value, label: selectedCountry.label}}
            ...
            <SelectContent insets={contentInsets} className="w-full max-h-96 border border-border bg-secondary">
                <FlatList
                    data={countryData}
                    renderItem={({item}) => (
